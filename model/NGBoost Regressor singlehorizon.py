...
DROP_35_LEAST_IMPORTANT: bool = True
...
model_files: list[str] = glob.glob(f"{model_cache_prefix}/model_*.p")

cached_model_available: bool = len(model_files) > 0
...
if USE_CACHED_MODEL and cached_model_available:
    ...
    # os delete model_cache_prefix folder
    ...
    model: str = (
        "model_" f"{time.day}.{time.month}.{time.year}_" f"{time.hour}:{time.minute:02}"
    )

    with open(f"{model_cache_prefix}/{model}.p", "wb") as file:
...
plt.rcParams.update({"font.size": 11 if LOG_TRANSFORM else 12})

if SAVE_FIGURES:
    plt.savefig(f"{figures_prefix}/NGBoost_predictions.png", dpi=300)
# TODO: clear plt cache here (so we don't bring it over to the next steps)
...
for index, country_id in enumerate(country_ids):
    ...
    # plt.plot(
    #     this_country_test['date'],
    #     this_country_test['predictions'],
    #     label=f'{country_name} Predicted XGB',
    #     color='red',
    #     linestyle='-',
    #     marker=''
    # )
...
if PLOT_ONE_FIGURE:
    ...
    for country_id in country_ids:
        ...
        fig.add_trace(
            go.Scatter(
                x=this_country_train["date"],
                y=this_country_train[target],
                name=f"{country_name} Train",
                mode="lines",
        ...
        fig.add_trace(
            go.Scatter(
                x=this_country_test["date"],
                y=this_country_test[target],
                name=f"{country_name} Test Actual",
                mode="lines",
        ...
        fig.add_trace(
            go.Scatter(
                x=this_country_test["date"],
                y=this_country_test["predictions"],
                name=f"{country_name} Predicted",
                mode="lines",
...
test_df_new[
    ...
    & (test_df_new["date"] == f"{prediction_year}-07")
]
...
if SAVE_PREDICTIONS:
    ...
    new_predictions.to_parquet(folder + f"/{folder_to_str}_Y{prediction_year}.parquet")
